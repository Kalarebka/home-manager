...
class AddTaskView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            return redirect(
                "board:add_task_fail",
                context={"message": "Something went wrong when adding the task"},
            )
...
class CreateBoardView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            redirect(
                "board:add_task_fail",
                {"message": "Something went wrong when adding the board"},
            )
...
class JoinBoardView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        return render(
            request, "board/join_board.html", context={"boards": boards_with_creators}
        )
...
def get_tasks_by_status(board: Board) -> namedtuple:
    todo = Task.objects.filter(board=board.id, status="todo").order_by(
        "priority", "-date_created"
    )
    wip = Task.objects.filter(board=board.id, status="wip")
    done = Task.objects.filter(board=board.id, status="done").order_by(
        "-date_completed"
    )
...
def send_message(
    ...
    message = Message.objects.create(
        from_user=from_user, to_user=to_user, title=title, message=message
    )
